...
def main():
    ...
    for group_id in indexes:
        ...
        path_log = DIR_LOG + '/' + str(group_id)
        dirs_log = [name for name in os.listdir(path_log)
                    if os.path.isdir(os.path.join(path_log, name))]
        path_res = DIR_RES + '/' + str(group_id)
        dirs_res = [name for name in os.listdir(path_res)
                    if os.path.isdir(os.path.join(path_res, name))]
        dirs_log.sort()
        dirs_res.sort()

        for (x, y) in zip(dirs_log, dirs_res):
            dir_log = os.path.join(path_log, x)
            dir_res = os.path.join(path_res, y)
            ...
            for (plan, fact, res) in paires:
                ...
                with open(os.path.join(dir_res, res), 'r') as f_res:
                    ...
                    other_feature['script_name'] = data_res['script_name']
                ...
                d_features['other'].append(other_feature)

        # pprint(d_features)
        with open(os.path.join(OUT_NAME, str(group_id)+'_'+'features'),
                  'w') as f:
